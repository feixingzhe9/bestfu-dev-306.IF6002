...
/*添加单元接口函数，与属性接口列表*/
/*格式：｛上电初始化函数接口，EEPROM数据初始化函数接口，单元验证函数接口，单元属性列表名,单元属性列表元素个数｝*/
/*以上接口函数，当单元不需要此功能时直接用NULL表示。单元属性列表名必须为一个确定*/
const Unit_st UnitTab[] = 
{
	{SysAttr_Init	, NULL			, NULL				, (AttrExe_st*)SysAttrTable		,Sys_UnitAttrNum	},	//系统共用属性，添加者请误修改
	{KeyInit		, KeyEepromInit	, KeyChecking_Attr  , KeyAttrTab					,IF6002_UnitAttrNum	},  //触摸
